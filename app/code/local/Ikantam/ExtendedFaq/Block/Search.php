...
class Ikantam_ExtendedFaq_Block_Search extends Mage_Core_Block_Template
{	
	...
	protected function getQuestionsCollection()
	{
		if (!$this->_collection) {
			...
			if ($this->getQuery()) {
				...
				$this->_collection->addFieldToFilter(array('question', 'answer'), array(
					array('like' => $query),
					array('like' => $query),
				));
			}
	...
	{
		if ($categoryId) {
			$collection = Mage::getModel('extendedfaq/faq')->getCollection()->setOrder('sort_order', 'asc')->addFieldToFilter('is_active', 1)->addFieldToFilter('category_id', $categoryId);
			...
			if ($this->getQuery()) {
				...
				$collection->addFieldToFilter(array('question', 'answer'), array(
					array('like' => $query),
					array('like' => $query),
				));
			}
