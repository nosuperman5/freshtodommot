 ...
 */
class Magestore_RewardPointsCoupon_Model_Coupon_Massgenerator extends Mage_Core_Model_Abstract
    implements Magestore_RewardPointsCoupon_Model_Coupon_CodegeneratorInterface
{
    /**
    ...
    public function generateCode()
    {
        ...
        if (!$format) {
            $format = Magestore_RewardPointsCoupon_Helper_Coupon::COUPON_FORMAT_ALPHANUMERIC;
        }
        $length  = max(1, (int) $this->getLength());
        $split   = max(0, (int) $this->getDash());
        $suffix  = $this->getSuffix()?'-'.$this->getSuffix():'';
        $prefix  = $this->getPrefix()?$this->getPrefix().'-':'';

        $splitChar = $this->getDelimiter();
        $charset = Mage::helper('rewardpointscoupon/coupon')->getCharset($format);

        $code = '';
        $charsetSize = count($charset);
        for ($i=0; $i<$length-  strlen($suffix)-  strlen($prefix); $i++) {
    ...
    public function getDelimiter()
    {
        if ($this->getData('delimiter')) {
            ...
            return Mage::helper('rewardpointscoupon/coupon')->getCodeSeparator();
        }
    }
    ...
    public function generatePool()
    {
        ...
        $chars = count(Mage::helper('rewardpointscoupon/coupon')->getCharset($this->getFormat()));
        ...
        for ($i = 0; $i < $size; $i++) {
            ...
            do {
                if ($attempt >= $maxAttempts) {
                    Mage::throwException(Mage::helper('rewardpointscoupon')->__('Unable to create requested Coupon Qty. Please check settings and try again.'));
                }
            ...
            $coupon->setId(null)
                ...
                ->setCreatedAt($now)
                ->setType(Magestore_RewardPointsCoupon_Helper_Coupon::COUPON_TYPE_SPECIFIC_AUTOGENERATED)
                ->setCode($code)
                ->save();
    ...
    {
        return !empty($data) && !empty($data['qty']) && !empty($data['template_id'])
            && !empty($data['length']) && !empty($data['format'])
            && (int)$data['qty'] > 0 && (int) $data['template_id'] > 0
            && (int) $data['length'] > 0;
    }
